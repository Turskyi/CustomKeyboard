...
open class DefaultKeyboardController(inputConnection: InputConnection):
    ...
    override fun handleKeyStroke(key: SpecialKey) {
        when(key) {
            SpecialKey.DELETE -> deleteNextCharacter()
            SpecialKey.BACKSPACE -> deletePreviousCharacter()
            SpecialKey.CLEAR -> clearAll()
            SpecialKey.FORWARD -> moveCursorForwardAction()
            SpecialKey.BACK -> moveCursorBackAction()
            else -> {
                /* If needed access to one of the SpecialKey's not listed here, override
    ...
    override fun maxCharacters()= MAX_CHARACTERS
}
