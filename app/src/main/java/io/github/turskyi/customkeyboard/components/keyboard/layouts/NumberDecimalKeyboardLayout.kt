...
class NumberDecimalKeyboardLayout(context: Context, controller: KeyboardController?) :
    ...
    override fun createRows(): List<LinearLayout> {
        ...
        rowOne.add(createButton("⌫", columnWidth, SpecialKey.BACKSPACE))
        ...
        if (hasNextFocus) {
            rowTwo.add(createButton("Next", columnWidth, SpecialKey.NEXT))
        } else {
            rowTwo.add(createButton("Done", columnWidth, SpecialKey.DONE))
        }
        ...
        rowFour.add(createButton("⇦", columnWidth, SpecialKey.BACK))
        rowFour.add(createButton("0", columnWidth, '0'))
        rowFour.add(createButton("⇨", columnWidth, SpecialKey.FORWARD))
        rowFour.add(createButton("Clear", columnWidth, SpecialKey.CLEAR))
