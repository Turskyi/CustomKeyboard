...
class QwertyKeyboardLayout(context: Context, controller: KeyboardController?) :
    ...
    override fun createRows(): List<LinearLayout> {
        if (symbolsState !== SymbolState.SYMBOLS_DISABLED) {
            ...
            return when(capsState) {
                CapsState.CAPS_DISABLED -> createLowerCaseRows()
                CapsState.CAPS_ENABLED -> createUpperCaseRows()
                CapsState.CAPS_LOCK_ENABLED -> createUpperCaseRows()
            }
        }
    }
    ...
    private fun createLowerCaseRows(): List<LinearLayout> {
        ...
        rowTwo.add(createButton("⌫", columnWidth, SpecialKey.BACKSPACE))
        ...
        if (hasNextFocus) {
            rowThree.add(createButton(
                    "Next", (columnWidth * 1.5f), SpecialKey.NEXT))
        } else {
            rowThree.add(createButton(
                    "Done", (columnWidth * 1.5f), SpecialKey.DONE))
        }
        ...
        rowFive.add(createButton(
                "Symbols", (columnWidth * 2.0f), SpecialKey.SYMBOL))
        rowFive.add(createButton("", columnWidth * 7.0f, ' '))
        rowFive.add(createButton("⇦", columnWidth, SpecialKey.BACK))
        rowFive.add(createButton("⇨", columnWidth, SpecialKey.FORWARD))
    ...
    private fun createSymbolsOneRows(): List<LinearLayout> {
        ...
        val rowFive = ArrayList<View>()
        rowFive.add(createButton(
                "Sym (1/2)", (columnWidth * 2.0f), SpecialKey.SYMBOL))
        rowFive.add(createButton("", columnWidth * 7.0f, ' '))
        rowFive.add(createButton("⇦", columnWidth, SpecialKey.BACK))
        rowFive.add(createButton("⇨", columnWidth, SpecialKey.FORWARD))
    ...
    private fun createSymbolsTwoRows(): List<LinearLayout> {
        ...
        val rowFive = ArrayList<View>()
        rowFive.add(createButton(
                "Sym (2/2)", (columnWidth * 2.0f), SpecialKey.SYMBOL))
        rowFive.add(createButton("", columnWidth * 7.0f, ' '))
        rowFive.add(createButton("⇦", columnWidth, SpecialKey.BACK))
        rowFive.add(createButton("⇨", columnWidth, SpecialKey.FORWARD))
    ...
    private fun createNumbersRow(): LinearLayout {
        ...
        row.add(createButton("Del", columnWidth, SpecialKey.DELETE))
    ...
    private fun createCapsButton(): Button {
        ...
        return when(symbolsState) {
            SymbolState.SYMBOLS_DISABLED -> {
                when(capsState) {
                    CapsState.CAPS_DISABLED -> {
                        createButton("⇧", columnWidth, SpecialKey.CAPS)
                    }
                    CapsState.CAPS_ENABLED -> {
                        createButton("⬆", columnWidth, SpecialKey.CAPS)
                    }
                    CapsState.CAPS_LOCK_ENABLED -> {
                        val button = createButton(
                                "⇧", columnWidth, SpecialKey.CAPS)
                        ComponentUtils.setBackground(
                                button, Color.parseColor("#33CCFF"))
                        return button
                    }
                }
            }
            SymbolState.SYMBOL_ONE_DISPLAYED -> createButton(alphaText, columnWidth, SpecialKey.ALPHA)
            SymbolState.SYMBOL_TWO_DISPLAYED -> createButton(alphaText, columnWidth, SpecialKey.ALPHA)
        }
    }
}